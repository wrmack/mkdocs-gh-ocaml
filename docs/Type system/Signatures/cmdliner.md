...
`cmdliner` is a library for wrapping a CLI command in additional information relating to its documentation. When the executable is run with `--help` it display a man (manual) page. If the executable is run without the appropriate arguments a short help will be displayed.
...
The tool will print a message `MSG` `COUNT` times. The help for the tool  will look like:
...
- `Term.(body)` means the module `Term` is opened in the body between the brackets. 

- `Term.const` has type `'a -> 'a t`.  It takes a type `'a` and returns a type `'a t`. In other words it lifts the value with type `'a` into the Term context.
